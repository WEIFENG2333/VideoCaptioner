...
class ASRData:
    ...
    def from_ass(ass_str: str) -> "ASRData":
        ...
        # 从ASS头部注释中提取副标题布局信息
        layout_match = re.search(r"; Layout: (.+)", ass_str)
        if layout_match:
            layout_str = layout_match.group(1)
        else:
            layout_str = SubtitleLayoutEnum.ORIGINAL_ON_TOP.value

        # 根据布局确定 Default/Secondary 与原文/译文的映射关系
        # TRANSLATE_ON_TOP: Default=译文, Secondary=原文
        # ORIGINAL_ON_TOP:   Default=原文, Secondary=译文
        if layout_str == SubtitleLayoutEnum.TRANSLATE_ON_TOP.value:
            default_is_original = False
        else:
            default_is_original = True
