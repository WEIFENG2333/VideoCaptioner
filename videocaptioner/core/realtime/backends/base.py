...
class LiveTranscriber(ABC):
    ...
    def _reconnect_with_backoff(self, reason, *, attempt, is_stopping, logger, name) -> bool:
        ...
        （配额满 / 限流等 "accept-then-drop"）里 attempt 每次都建连成功、``fails`` 永不累加，必须靠
        跨调用累计的 ``_reconnect_streak`` 兜底终止，否则会每 0.5s 无限重连、永不出字也永不报错。
        ``is_stopping`` 为真或 ``_reconnect_wake`` 被 set（stop()）时立即中断。
        """
        now = time.monotonic()
        ...
        if self._reconnect_streak > self._RECONNECT_MAX_FAILS:
            self._emit_error(
                f"{name} 反复连上即断（已 {self._reconnect_streak} 次，疑似配额 / 限流），停止续录：{reason}")
            return False
        ...
        # 偶发单断立即重连；抖动期（连上又秒断）先等一个最小间隔，否则会全速重连空转打满 CPU。
        delay = 0.5 if flaky else 0.0
        fails = 0
