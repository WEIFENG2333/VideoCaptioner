...
def _run_checks(config: dict, *, check_api: bool = False) -> list[Check]:
    ...
    checks.append(
        _check_command(
            "ffmpeg", "Required for audio extraction, timing fit, muxing, and hard subtitles."
        )
    )
...
def _check_ytdlp() -> Check:
    ...
    try:
        ...
        return Check(
            "yt-dlp", "ok", "embedded yt_dlp module" + (f" ({version})" if version else "")
        )
    except Exception:
        return Check(
            "yt-dlp",
...
def _check_dubbing(config: dict) -> list[Check]:
    ...
    if preset_name:
        ...
        checks.append(
            Check(
                "dubbing.preset",
                "warn",
                "No dubbing preset configured",
                "Run 'videocaptioner config set dubbing.preset edge-cn-female'",
            )
        )
    if provider != "edge" and not get(config, "dubbing.api_key", ""):
        checks.append(
            Check(
                "dubbing.api_key",
                "warn",
                "Dubbing TTS API key is missing",
                "Run 'videocaptioner config set dubbing.api_key <key>'",
            )
        )
    if provider not in {"siliconflow", "gemini", "edge"}:
        checks.append(
            Check(
                ...
                "Use siliconflow, gemini, or edge",
            )
        )
    ...
    if voice_error:
        checks.append(
            Check(
                "dubbing.voice", "error", voice_error, "Use a preset or a provider-supported voice"
            )
        )
    else:
    ...
    if timing not in {"balanced", "strict", "natural", "none"}:
        checks.append(
            Check(
                "dubbing.timing",
                "error",
                f"Invalid timing: {timing}",
                "Use balanced, strict, natural, or none",
            )
        )
...
def _check_api(config: dict) -> list[Check]:
    ...
    checks.append(
        Check(
            "api.dubbing",
            "warn",
            "--check-api is currently limited to configuration validation",
            "Run a short 'videocaptioner dub sample.srt' to verify billing/provider access",
        )
    )
    return checks
